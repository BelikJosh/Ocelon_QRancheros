...
export default function WalletScreen() {
  ...
  const [serverReceiver, setServerReceiver] = useState<string | null>(null); // NEW
  ...
  // NEW: traer wallets del server al montar
  useEffect(() => {
    (async () => {
      try {
  ...
  // Listener deep link finish
  useEffect(() => {
    const sub = Linking.addEventListener('url', async (evt) => {
      try {
        const url = evt.url;
        const parsed = Linking.parse(url);
        const interact_ref = (parsed.queryParams?.interact_ref as string) || '';
        if (!interact_ref || !flow.incomingId || !flow.continueUri || !flow.continueAccessToken) return;

        setLoading(true); // NEW
        const { outgoingPayment } = await opApi.finishOutgoing({
        ...
        Alert.alert('Error', e?.message ?? 'No se pudo finalizar el pago');
      } finally {
  ...
  const confirmar = async () => {
    try {
      // Validación útil: si el QR trae "to" y no coincide con el receiver del server, avisar
      ...
      // 1) Monto en minor units (usa el scale del receiver del server; aquí asumimos 2)
      const minor = Math.round(Number(summary.monto || '0') * 100).toString();

      // 2) Crear Incoming Payment en el receiver configurado en el server
      const { incomingPayment } = await opApi.createIncoming(minor);

      // 3) Iniciar flujo OUTGOING del sender
      ...
      // 4) Abrir consentimiento (IdP redirige a ocelon://pay/finish?interact_ref=...)
      ...
      >
        ...
        <View style={[s.header, { maxWidth: MAX_W }]}>
          ...
          <View style={[s.badge, { borderRadius: 999, paddingHorizontal: hs(10), paddingVertical: vs(6) }]}>
            ...
            <Text style={[s.badgeText, { fontSize: ms(12) }]}>
              {summary.scheme}
            </Text>
          </View>
        </View>
        ...
        <View
          style={[
            s.amountCard,
            { maxWidth: MAX_W, borderRadius: RADIUS, padding: hs(16) },
          ]}
        >
          ...
          {serverReceiver && summary.walletDestino !== 'N/D' && summary.walletDestino !== serverReceiver && (
            <Text style={{ color: '#ffcf99', marginTop: 6 }}>
              Se usará el receiver del servidor: {shorten(serverReceiver)}
            </Text>
          )}
        </View>
        ...
        <View style={[s.actions, { maxWidth: MAX_W, gap: vs(10) }]}>
          <TouchableOpacity
            style={[
              s.btn,
              s.confirm,
              { borderRadius: hs(12), paddingVertical: vs(14), opacity: loading ? 0.7 : 1 },
            ]}
            onPress={confirmar}
            disabled={loading}
          >
        ...
        <View style={{ maxWidth: MAX_W, width: '100%' }}>
          <View style={[s.helper, { borderRadius: hs(12), padding: hs(10) }]}>
            ...
            <Text style={[s.helperText, { fontSize: ms(12) }]}>
              Este flujo crea un Incoming en el receiver del backend, luego un Outgoing desde el sender tras tu consentimiento.
            </Text>
          </View>
        </View>
      </ScrollView>
...
function DetailRow({
  ...
  return (
    <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: hs(10), paddingVertical: 6 }}>
      ...
      <Text
        style={{ color: '#fff', fontWeight: '600', marginLeft: 'auto', maxWidth: '66%', textAlign: 'right' }}
        numberOfLines={2}
      >
        {value ?? '—'}
      </Text>
